...
class UserModelTestCase(unittest.TestCase):
    ...
    def test_valid_email_change_token(self):
        ...
        db.session.add(u)
        db.session.commit()
        token = u.generate_email_change_token("new_test@test.com")
        self.assertTrue(u.change_email(token))
        self.assertTrue(u.email == "new_test@test.com")

    def test_invalid_email_change_token(self):
        u1 = User(email="test1@test.com")
        u2 = User(email="test2@test.com")
        db.session.add(u1)
        db.session.add(u2)
        db.session.commit()
        token = u1.generate_email_change_token("new_test@test.com")
    ...
    def test_duplicate_email_change_token(self):
        ...
        db.session.add(u2)
        db.session.commit()
        token = u2.generate_email_change_token("test1@test.com")
        self.assertFalse(u2.change_email(token))
        self.assertTrue(u2.email == "test2@test.com")
    ...
    def test_gravatar(self):
        ...
        self.assertTrue("https://secure.gravatar.com/avatar/" + 
                        "b642b4217b34b1e8d3bd915fc65c4452" in gravatar_ssl)

    def test_follows(self):
        u1 = User(email="test1@test.com", password="cat")
        u2 = User(email="test2@test.com", password="dog")
        db.session.add(u1)
        db.session.add(u2)
        db.session.commit()
