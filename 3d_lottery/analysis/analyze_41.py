...
class Analyze(object):
    ...
    def __init__(self):
        ...
        #self._csv_fn = "../data/3d_lottery_2018.csv"
    ...
    def calc(self):
        ...
        按4推1的方式计算中奖情况
        """
        for index in range(len(self._data)-4):
            ...
            five_prize_num = (int(five[2]), int(five[3]), int(five[4]))
            ...
            if len(diff_set) == 2:
                # 中奖了
                if diff_set.pop() in five_prize_num and diff_set.pop() in five_prize_num:
                    self._winning_record.append((five[1], self._purchase_num_per_circle+1))
                    self._purchase_num_per_circle = 0
                    self._prize_num += 1
