...
class Analyze(object):
    ...
    def calc(self):
        ...
        按7推1的方式计算中奖情况
        """
        for index in range(len(self._data)-7):
            ...
            seven = self._data[index+6]
            eight = self._data[index+7]
            
            eight_prize_num = (int(eight[2]), int(eight[3]), int(eight[4]))
            ...
            if len(diff_set) == 2:
                # 中奖了
                if diff_set.pop() in eight_prize_num and diff_set.pop() in eight_prize_num:
                    self._winning_record.append((seven[1], self._purchase_num_per_circle+1))
                    self._purchase_num_per_circle = 0
                    self._prize_num += 1
